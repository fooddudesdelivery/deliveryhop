	...
	function Receiver(){
	  ...
	  function newOrder(params){
		  ...
		  // setTimeout(function(){
		   $('#header').hide();
			ReactDOM.render(React.createElement(window.NewOrder,{categories_id:window.categories_id,orders_id:params.params}), document.getElementById('new_order_data'));
		   $( ":mobile-pagecontainer" ).pagecontainer( "change", "#new_order", { role: "dialog", changeHash:false, transition:'flip'} ); 
		  // },300); 
		  console.log('IN NEW ORDER');
//		  	$.ajax({
//				url: 'http://deliverhop.app/cordova/www/receive.php',
//				dataType: 'json',
//				cache: false,
//				type:'POST',
	  ...
	  function mediaPlayer(){
		 console.log('IN MEDIA PLAYER');
		  ...
		  function getMediaURL(s){
			  console.log('FORMENPLATZ '+device.platform.toLowerCase());
			  if(device.platform.toLowerCase() === "android" || device.platform.toLowerCase() === "amazon-fireos") return "/android_asset/www/" + s;
			  return s;
		  }
	  }
	  ////////
