...
window.NewOrder = React.createClass({
   ...
   orderAccepted:function(){
	 // $('#header').show();
	  ...
	  SendAjax({key:'accept_order',params:{orders_id:this.props.orders_id,eta:$('#etaslide').val()}},function(){
		window.sync();  
	  });
   },
  render: function() { 
    return (
	  <div>
		<div className="new-order-header">
		   <div>Order #{this.props.orders_id}</div>
		</div>
  ...
  render: function() {
	...
	if(this.props.orders.length){
	  orderNodes = this.props.orders.map(function(order, index) {
		return (
		   <Order timeframe={this.props.timeframe} extras={order.extras} orders_info={order.orders_info}  products={order.products} key={index} setOrdersId={this.props.setOrdersId} />
		);
	  }.bind(this));
	}
    return (
	  ...
	  <fieldset className="info">
		 <legend>Info</legend>
			 <div className="info-content container-fluid">
			   ...
			   <span className="col-xs-6">{this.props.orders_info.date_deliver}</span>
	...
	render: function render() {
		...
		<div  className="fast_form">
		  <div className="container" ref="placed_order" hidden>
				...
				<div className="col-xs-12 btn btn-default" ref="refr">Place Another</div>
		  </div>
	  ...
	  <button type="button" id="main_submit" onClick={this.placeCashOrder} className="col-xs-12  btn btn-default subbtn">Place Order</button>
	  </form>
	  ...
	  <div className="form-group col-xs-4">
		  ...
		  <button type="button" onClick={this.placeCreditOrder} className="col-xs-12 btn btn-default subbtn">Place Order</button>
