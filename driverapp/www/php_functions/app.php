<?php
	class RestaurantBase{
		...
		private function getTaxFromConfig(){
			...
			$payment_types = array('invoice'=>$invoice,'credit'=>$rest_config['delivery']['credit'],'cash'=>$rest_config['delivery']['cash']);
...
//			}
		...
		public function checkNewOrder(){
			//do db stufff here
			return json_encode(array('success'=>true,'orders_id'=>$this->primary_params));
		}
		...
		private function addCharge(){
			...
			$orders_table = "update orders set order_total=".$total.",order_tax=".$tax." where orders_id =".$this->primary_params['orders_id'];
			...
			$note->bindValue(':note','Adjustment '.$type.round(floatval($this->primary_params['adjustment']),2).' '.$this->primary_params['note'], PDO::PARAM_STR);
		...
		foreach($reportTable as &$rt){
			...
			$rt['order_total']=money_format('%i',floatval($rt['order_total'])-$bev);
			$tot+=$rt['order_total'];
		}
...
//		LIMIT 0 , 999999";
		...
		private function pastOrders(){
			...
			return json_encode($this->getOrder($orders_id_list));
		}
		...
		public function getOrder($orders_id_list){
		 ...
		 // if(in_array(148283,$orders_id_list)){
		  ...
		  //BOF sql looping to array
			  ...
			  $history = $this->db->query('select orders_id from orders_status_history where orders_id in ('.$orders_id_string.') and orders_status_id=4')->fetchAll(PDO::FETCH_ASSOC);
			  ...
			  try{
				  ...
				  foreach ($a as $ax){
					  	if($ax['orders_status']==9 || $ax['updated_from_future']==1){
							$display_date = date('m/d g:ia',strtotime($ax['date_deliver'])-1200);
						}else{
						  ...
						  $order[]=array('orders_info'=>
										  array('orders_id'=>$ax['orders_id'],
												...
												'customers_name'=>$ax['customers_name'],
