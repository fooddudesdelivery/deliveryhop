<?php
	...
	if(defined('_DB_SERVER')){
		...
		mail('sunil.kalwani@fooddudesdelivery.com','Connection Failure',"/public_html/fax_and_cloud.php, 20");
	...
	class AutoSendNew{
		...
		private $from='service@staging.fooddudesdelivery.com';
		...
		function __construct(){
			...
			try {
				if(defined('_DB_SERVER')){
					$this->db = new PDO("mysql:host="._DB_SERVER.";dbname="._DB_DATABASE, _DB_SERVER_USERNAME, _DB_SERVER_PASSWORD);
				}else{
					mail('sunil.kalwani@fooddudesdelivery.com','Connection Failure',"/public_html/fax_and_cloud.php, __construct");
		...
		private function sendPdfFax(){
			...
			if($main_backup_check['fax']==1 || $main_backup_check==null){
				...
				$fax_from='fax@staging.fooddudesdelivery.com';
				...
				$fax_from='fooddudestaging@server.staging.fooddudesdelivery.com';
			...
			/*$to = "sunilkalwani2007@gmail.com";

			$subject = "Test Fax ".date("Y-m-d g:i:s");*/
		    ...
		    <tr >
     		...
     		for($r=0;$r<count($order_array);$r++){
	  		...
	  		if($order_array[0]['pickup_order']==1){
				...
				/*if(!empty($orders_total_arr['ot_shipping'])){

					$r_invoice.='<tr>

						<td></td>

						<td style="font-size:14px">Delivery: </td>

						<td style="font-size:14px">&nbsp;'.$orders_total_arr['ot_shipping']['text'].'</td>
...
/*
				...
				if(!empty($orders_total_arr['ot_tip'])){

					$r_invoice.='<tr>

						<td></td>

						<td style="font-size:14px">Gratuity: </td>

						<td style="font-size:14px">&nbsp;'.$orders_total_arr['ot_tip']['text'].'</td>
...
*/
	...
	//$db->Execute("UPDATE orders SET orders_status='2', last_modified = now() WHERE orders_id = '".$order['orders_id']."'");

    //$db->Execute('insert into orders_status_history (orders_id,orders_status_id,date_added,updated_by) values  ('.$order['orders_id'].',2,now(),"'.get_admin_name(1).'")');
