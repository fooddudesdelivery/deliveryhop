...
class Location_new{
    ...
    public function runLocation(){
        ...
        if(count($v)==0){
           print_r(json_encode(array('code'=>'error','message'=>'Sorry your address is out of range')));
            return;
        }
		...
		if(is_array($main)){
			if($main[0]['categories_status']==0){
				...
				if(count($this->expanded_city_list)==0){
					print_r(json_encode(array('code'=>'error','message'=>'Sorry you are out of range')));
					die;
				}
			}
		}else{
	...
	private function getLocationConfig(){
		...
		$rates_ranges_query="
		SELECT c.categories_id, json_range, tier_5_distance as max_travel_distance, tier_5_time as max_travel_time ,comments,lat,lng,categories_name,c.categories_status,
		tier_1_distance ,  
		tier_2_distance ,  
		tier_3_distance ,  
		tier_4_distance ,  
	...
	private function analyzeRates(){
		...
		foreach($this->routes as $key=>&$dist){
			...
			$rf = $this->rates['tiers'];
			if(($dist['distance'] > 0 && $dist['distance'] < $rf[0]['distance']) || ($dist['duration']>0 && $dist['duration'] < $rf[0]['duration'])){
				$delivery_fee=floatval($rf[0]['price']);
				//$delivery_duration=$rf[0]['duration'];
			}elseif(($dist['distance'] > $rf[0]['distance'] && $dist['distance'] < $rf[1]['distance']) || ($dist['duration']>$rf['tier_1_time'] && $dist['duration'] < $rf['tier_2_time'])){
				$delivery_fee=floatval($rf[1]['price']);
				//$delivery_duration=$rf[1]['duration'];
			}elseif(($dist['distance'] > $rf[1]['distance'] && $dist['distance'] < $rf[2]['distance']) || ($dist['duration']>$rf['tier_2_time'] && $dist['duration'] < $rf[2]['duration'])){
				$delivery_fee=floatval($rf[2]['price']);
				//$delivery_duration=$rf[2]['duration'];
			}elseif(($dist['distance'] > $rf[2]['distance'] && $dist['distance'] < $rf[3]['distance']) || ($dist['duration']>$rf[2]['duration'] && $dist['duration'] < $rf[3]['duration'])){
				$delivery_fee=floatval($rf[3]['price']);
				//$delivery_duration=$rf[3]['duration'];
			}elseif(($dist['distance'] > $rf[3]['distance'] && $dist['distance'] < $rf[4]['distance']) || ($dist['duration']>$rf[3]['duration'] && $dist['duration'] < $rf[4]['duration'])){
				$delivery_fee=floatval($rf[4]['price']);
				//$delivery_duration=$rf[4]['duration'];
			}elseif($dist['distance']>$rf[4]['distance'] || $dist['duration']>$rf[4]['duration']){
				unset($this->routes[$key]);
				continue;
			}
			/*if($delivery_fee==0){
			...
			if($dist['distance'] < 2 && $dist['categories_id'] == 14094){
				$delivery_fee = 2.99;
			}
			$dist['delivery_fee']=$delivery_fee;
			//$dist['duration']=$delivery_duration;
		}
