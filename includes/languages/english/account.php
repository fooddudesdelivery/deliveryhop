<?php
/**
 * @package languageDefines
 * @copyright Copyright 2003-2006 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: account.php 3595 2006-05-07 06:39:23Z drbyte $
 */

define('NAVBAR_TITLE', 'My Account');
define('HEADING_TITLE', 'My Account Information');
define('TEXT_GV_REDEEM_ID', 'Certificate Code');
define('TEXT_GV_REDEEM_INFO', 'Redeem Gift Certificate');

define('OVERVIEW_TITLE', 'Overview');
define('OVERVIEW_SHOW_ALL_ORDERS', '(show all orders)');
define('OVERVIEW_PREVIOUS_ORDERS', 'Previous Orders');
define('TABLE_HEADING_DATE_ORDER', 'Order Date');
define('TABLE_HEADING_DATE_DELIVER', 'Delivery Date');
define('TABLE_HEADING_ORDER_NUMBER', 'No.');
define('TABLE_HEADING_SHIPPED_TO', 'Deliver To');
define('TABLE_HEADING_STATUS', 'Status');
define('TABLE_HEADING_TOTAL', 'Total');
define('TABLE_HEADING_VIEW', 'View');

define('MY_ACCOUNT_TITLE', 'My Account');
define('MY_ACCOUNT_HISTORY', 'Order History');
define('MY_ACCOUNT_INFORMATION', 'Account Settings');
define('MY_ACCOUNT_ADDRESS_BOOK', 'Address Book');
define('MY_ACCOUNT_PASSWORD', 'Change Password');

define('MY_ORDERS_TITLE', 'My Orders');
define('MY_ORDERS_VIEW', 'View the orders I have made.');
define('GV_SEND_ZM', 'You have a Gift Certificate Balance you can either send or spend this.');
define('EMAIL_NOTIFICATIONS_TITLE', 'Email Notifications');
define('EMAIL_NOTIFICATIONS_NEWSLETTERS', 'Subscriptions');
define('EMAIL_NOTIFICATIONS_PRODUCTS', 'Notifications');
?>