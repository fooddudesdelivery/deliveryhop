...
class order extends base {
  ...
  function __construct($order_id = '') {
    ...
    $this->customer = array();
    $this->delivery = array();
    /*New Code For Customer Mail Total*/
    $this->total_price = "0.00";
    $this->excluded_items = array("FOODDUDEMISC", "BEVERAGE");
    /*New Code For Customer Mail Total*/
  ...
  function query($order_id) {
    ...
    $order_query = "select customers_id, customers_name, customers_company,
                         ...
                         customers_postcode, customers_state, customers_country,date_deliver,
    ...
    $this->info = array('currency' => $order->fields['currency'],
            ...
            'date_deliver' => $order->fields['date_deliver'],
  ...
  function smsSinchMsgOrder($phone_number,$message)
  {
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
    ...
    // zm edit
    $categories_id=zm_check_cart();
    if($categories_id==0){
      $categories_id=last_resort_category_id();
      if($categories_id==0){
        $categories_id=1916;
      }
    }
    $_SESSION['categories_name'] = zen_categories_lookup($categories_id);
    unset($_SESSION['real_menu']);
    ...
    if($_SESSION['delivery_time']==1){
      ...
      $updated_from_future=0;
      $orders_status=1;
    }else{
      $dev_time = $_SESSION['delivery_time'];
      $orders_status=9;
      $updated_from_future=1;
    }
    if($_SESSION['is_gift_card_non_rewards'] || $_SESSION['is_gift_card']){
      $orders_status=14;
      $this_is_gift_card=true;
    }
    if(isset($_SESSION['customers_lat_lng'])){
      $lat =  round($_SESSION['customers_lat_lng'][0],10);
      $lng = round($_SESSION['customers_lat_lng'][1],10);
    }else{
      $lat = 0;
      $lng = 0;
    }
    $date_deliver = date('Y-m-d H:i:s',$dev_time);
    $delivery_duration = $restaurant_info[$categories_id]['duration'];
    $delivery_distance = $restaurant_info[$categories_id]['distance'];
    if($this->info['shipping_method']=='Delivery ()'){
      //$this->info['shipping_method']='Delivery';
    }
    for ($i=0, $n=sizeof($zf_ot_modules); $i<$n; $i++) {
        if($zf_ot_modules[$i]['code']=='ot_tax'){
          $this->info['tax']=floatval($zf_ot_modules[$i]['value']);
        }
    }
    // zm edit
    $sql_data_array = array('customers_id' => $_SESSION['customer_id'],
                            ...
                            'customers_telephone' => preg_replace("/[^0-9]/","",$this->customer['telephone']),
              ...
              'updated_from_future' => $updated_from_future,
              'date_deliver' => $date_deliver,
              'cash_order' => $cash_order,
              'customers_lat' => $lat,
              'customers_lng' => $lng,
              'duration_to_deliver' => $delivery_duration,
              'distance_to_deliver' => $delivery_distance,
    ...
    if ($_SESSION['COWOA']) $sql_data_array[COWOA_order] = 1; // FEAC
      ...
      $dup_arr = getRestaurantCopies();
    if(is_array($dup_arr)){
      if(in_array(intval($categories_id),array(16062,16139,18137,19461,16057,18798))){
      $this->smsSinchMsgOrder('+19705967281','New Order #' . $insert_id . ' - Montrose');
      }
      if(in_array(intval($categories_id),array(19103,28,17344,19062,15,30,23578,21068,23457,12965,14113,31,24920,11939,25551,25550,17,13218,14317,14329,14145,34,13019,13036,36,19,22880,14105,20))){
      $this->smsSinchMsgOrder('+13204933806','New Order #' . $insert_id . ' - Twin Cities');
      $this->smsSinchMsgOrder('+19788869809','New Order #' . $insert_id . ' - Twin Cities');    
      }
    if(in_array(intval($categories_id),array(26832))){
      $this->smsSinchMsgOrder('+19522246097','New Food Dudes Order #' . $insert_id . ' - Maple Grove');
    $this->smsSinchMsgOrder('+13202415374','New Food Dudes Order #' . $insert_id . ' - Maple Grove');
      }   
      if(in_array(intval($categories_id),array(22546,21862,21500,22551,23782,21509,22791,25770,22726,21477,22337,25522,25510,21495))){
      $this->smsSinchMsgOrder('+13202673510','New Order #' . $insert_id . ' - Brainerd');
      } 
    if(in_array(intval($categories_id),array(22556,16431,16339,27173,16413,16421,16360,16368,25758,25735,16637,20986,26552,16330,18187,22356,16651,20432))){
    $this->smsSinchMsgOrder('+12182445553','New Order #' . $insert_id . ' - Alexandria');
      }   
    if(in_array(intval($categories_id),array(27212,27344,27417,27809,27935,28026))){
    $this->smsSinchMsgOrder('+13202673510','New Order #' . $insert_id . ' - Willmar');    
      }     
    if(in_array(intval($categories_id),array(25681,25704,24370,25709,24368,24353,24374,24372,27988,24668,26313,25043,24592,25499,24664,24378,27769,24366,26744,24376))){
      $this->smsSinchMsgOrder('+13202900819','New Order #' . $insert_id . ' - Mankato');
      }   
    if(in_array(intval($categories_id),array(28061,27832,27963,27904,27614))){
      $this->smsSinchMsgOrder('+16515002573','New Order #' . $insert_id . ' - Rochester');
      $this->smsSinchMsgOrder('+13202900819','New Order #' . $insert_id . ' - Rochester');    
      }               
    }
    ...
    if($this->info['comments']!=''){
      $db->Execute('insert into notes (order_id,note_type,note) values ('. $insert_id.',4,"'.addslashes(utf8_decode($this->info['comments'])).'")');
    }
    zen_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
    $this->notify('NOTIFY_ORDER_DURING_CREATE_ADDED_ORDER_COMMENT', $sql_data_array);
  //make temp_quick_draw_table here
  //$sql_quick_table=array('orders_id','customers_name','customers_telephone','');
  //mail('3203106216@tmomail.net','New Order',$insert_id);
    return $insert_id;
  }
  function create_add_products($zf_insert_id, $zf_mode = false) {
    global $db, $currencies, $order_total_modules, $order_totals, $totalprice;/*totalprice Added For Customer Mail Total*/
    // initialized for the email confirmation
    $this->products_ordered = '';
    $this->products_ordered_html = '';
    $this->subtotal = 0;
    $this->total_tax = 0;
    /*$this->totalprice Added For Customer Mail Total*/
    $this->totalprice = 0;
    /*$this->totalprice Added For Customer Mail Total*/
  ...
  if($_SESSION['real_tax'] && $this->products[$i]['tax']>0){
    $new_tax = $_SESSION['real_tax'];
  }else{
    $new_tax = $this->products[$i]['tax'];
  }
      ...
      if (isset($this->products[$i]['attributes'])) {
        ...
        for ($j=0, $n2=sizeof($this->products[$i]['attributes']); $j<$n2; $j++) {
          ...
          $this->products_ordered_attributes .= "\n" . '- ' . zen_decode_specialchars($this->products[$i]['attributes'][$j]['value']);
        }
      }
    ...
    /* END: ADD MY CUSTOM DETAILS */
      ...
      /*New Code For Customer Mail Total*/
      if(!in_array($this->products[$i]['model'], $this->excluded_items) && trim($this->info['shipping_method']) == 'Delivery'){
        $tmp = $currencies->display_price($this->products[$i]['final_price'], $this->products[$i]['tax'], $this->products[$i]['qty']);
        $tmp = substr($tmp, 1);
        $this->totalprice += $tmp;
        $totalprice += $tmp;
  ...
  function send_order_email($zf_insert_id, $zf_mode = FALSE) {
    global $currencies, $order_totals, $totalprice;/*New Code For Customer Mail Total*/
    ...
    $html_msg=array();
      ...
      $e_dev_time = zm_date($_SESSION['delivery_time']);
    }
    if($_SESSION['is_gift_card'] || $_SESSION['is_gift_card_non_rewards'] ){
      $email_stuff=
      EMAIL_TEXT_ORDER_NUMBER . ' ' . $zf_insert_id . "\n" .
    EMAIL_TEXT_DATE_ORDERED . ' ' . date('m/d/Y',strtotime('now')) ;
    }else{
      $email_stuff= 'Restaurant: ' . ' ' . $_SESSION['categories_name'] . "\n" .
      EMAIL_TEXT_ORDER_NUMBER . ' ' . $zf_insert_id . "\n" .
    EMAIL_TEXT_DATE_ORDERED . ' ' . date('m/d/Y',strtotime('now')) . "\n" .
    EMAIL_TEXT_DATE_DELIVER . ' ' . $e_dev_time . "\n\n" ;
    }
    ...
    $_SESSION['new_order_id']=$zf_insert_id;
      // END FEAC
    ...
    unset($_SESSION['categories_name']);
    $_SESSION['new_order_id']=$zf_insert_id;
    //comments area
    if ($this->info['comments']) {
    ...
    /*New Code For Customer Mail Total*/
    if(trim($this->info['shipping_method']) == 'Delivery'){
      $email_order .= strip_tags('Total:') . ' $' . strip_tags($this->total_price) . "\n";
      $html_ot .= '<tr><td class="order-totals-text" align="right" width="100%">Total</td> ' . "\n" . '<td class="order-totals-num" align="right" nowrap="nowrap">$' .$this->total_price .'</td> </tr>' . "\n";
    }else{
      //order totals area
      ...
      for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
        if($order_totals[$i]['code']=='ot_shipping'){
          ...
          $ot_t = $order_totals[$i]['title'];
        }
        $email_order .= strip_tags($ot_t) . ' ' . strip_tags($order_totals[$i]['text']) . "\n";
        $html_ot .= '<tr><td class="order-totals-text" align="right" width="100%">' . $ot_t . '</td> ' . "\n" . '<td class="order-totals-num" align="right" nowrap="nowrap">' .($order_totals[$i]['text']) .'</td> </tr>' . "\n";
      }
    }
    /*New Code For Customer Mail Total*/
    ...
    if($_SESSION['is_gift_card_non_rewards'] || $_SESSION['is_gift_card']){
      $subjectx ='[GIFT CARD]' . ' ' . EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id;
    }else{
      $subjectx = SEND_EXTRA_NEW_ORDERS_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id;
    }
      zen_mail('', SEND_EXTRA_ORDER_EMAILS_TO,$subjectx,
