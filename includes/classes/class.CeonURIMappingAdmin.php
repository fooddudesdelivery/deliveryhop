...
class CeonURIMappingAdmin extends CeonURIMappingDBLookup
{
	 ...
	 * @access  public
	 */
	function CeonURIMappingAdmin($load_config = true)
	{
		parent::CeonURIMappingDBLookup();
	 ...
	 */
	function _convertStringForURI($text, $language_code)
	{
		global $db;
		 ...
		 */
		require_once(DIR_FS_CATALOG . DIR_WS_CLASSES . 'class.String.php');
		...
		if (!is_array($this->_remove_words)) {
			...
			$this->_remove_words = String::regexpReplace($this->_remove_words, '/\s/', '');
			
			// Escape any special characters in the selections of words
			$this->_remove_words = String::regexpReplace($this->_remove_words,
				'/([\/\-\\\!\?\$\^\[\]\|\*\.\(\)\{\}\=\<\>\:\+])/U', '\\\$1');
			...
			for ($i = 0, $n = sizeof($char_str_replacement_pairs); $i < $n; $i++) {
				...
				if (sizeof($current_char_str_replacement) == 2) {
					...
					$current_char_str_replacement[0] = String::regexpReplace($current_char_str_replacement[0],
						'/([\/\-\\\!\?\$\^\[\]\|\*\.\(\)\{\}\=\<\>\:\+])/U', '\\\$1');
		...
		if (sizeof($this->_remove_words) > 0) {
			...
			$remove_words_pattern =
				String::substr($remove_words_pattern, 0, String::length($remove_words_pattern) - 1);
			
			// Remove the words surrounded by spaces, replacing them with a single space
			$text = String::regexpReplace($text, '/' . $remove_words_pattern . '/i', ' ');
		...
		if (sizeof($this->_char_str_replacements) > 0) {
			foreach ($this->_char_str_replacements as $char_str_replacement) {
				// Remove the words surrounded by spaces, replacing them with a single space
				$text = String::regexpReplace($text, '/' . $char_str_replacement['char_str'] . '/i',
		...
		$text = String::transliterate($text, CHARSET, $language_code);
