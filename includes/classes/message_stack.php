...
class messageStack extends base {
  ...
  function add($class, $message, $type = 'error') {
    ...
    if (strlen($message) > 0) {
      if ($type == 'error') {
        $theAlert = array('params' => 'class="messageStack messageStackError larger"', 'class' => $class, 'text' => '' . $message);
      } elseif ($type == 'warning') {
        $theAlert = array('params' => 'class="messageStack messageStackWarning larger"', 'class' => $class, 'text' => '<i class="fa fa-info-circle"></i> ' . $message);
      } elseif ($type == 'success') {
        $theAlert = array('params' => 'class="messageStack messageStackSuccess larger"', 'class' => $class, 'text' => '<i class="fa fa-thumbs-up"></i> ' . $message);
      } elseif ($type == 'caution') {
        $theAlert = array('params' => 'class="messageStack messageStackCaution larger"', 'class' => $class, 'text' => '<i class="fa fa-info-circle"></i> ' . $message);
      } else {
        $theAlert = array('params' => 'class="messageStack messageStackError larger"', 'class' => $class, 'text' => $message);
      }
