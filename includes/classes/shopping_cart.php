...
class shoppingCart extends base {
 ...
 /* method to get product id array zm edit 
  *
  *
  *
 */
 ...
 // zm edit
     function get_product_id_array(){
	   $list = $this->get_product_id_list();
	   $ex_li = explode(',',$list);
	   $new_li = array();
	   foreach($ex_li as $ex){
	   if(!is_numeric($ex)){
		   $new_li_tmp= explode(':',$ex);
		   $new_li[] = $new_li_tmp[0];
	   }else{
		   $new_li[] = $ex;
	   }
  ...
  function add_cart($products_id, $qty = '1', $attributes = '', $notify = true) {
	...
	//zm edit to prevent different cat adding
	//this section may be unnceccisary as it is in actionAddCart
//	$cat_id = zm_check_cart();
//	if($cat_id!=0){
//		$new_cat_id=zm_get_restaurant_id_from_product($products_id);
//		if($new_cat_id!=$cat_id){
//			$messageStack->add_session('header',ONLY_ONE_RESTAURANT,'error');
//			return;
//		}
//	}
	//zm edit
...
// eof: adjust new quantity to be same as current in stock
  ...
  function remove($products_id) {
	...
	if(!is_numeric($products_id)){
		$new_li_tmp= explode(':',$products_id);
		$product_id = $new_li_tmp[0];
	}else{
		$product_id = $product_id;
	}
	$real_menu = $_SESSION['real_menu'][$product_id];
	if($real_menu==GIFT_CARD_PARENT_ID||$real_menu==REWARDS_PARENT_ID){
		unset($_SESSION['is_gift_card']);
		unset($_SESSION['is_virtual']);
		unset($_SESSION['is_gift_card_non_rewards']);
	}
	unset($_SESSION['real_menu'][$product_id]);
...
// display message if all is good and not on shopping_cart page
  ...
  function actionAddProduct($goto, $parameters) {
	...
	//$cat_id = zm_check_cart();
	//$new_cat_id=zm_get_restaurant_id_from_product($_POST['products_id']);
	
	if($_POST['real_menu']==REWARDS_PARENT_ID){
		if(!isset($_SESSION['customer_id'])){
				$messageStack->add_session('header',LOGIN_TO_USE_POINTS,'error');
				zen_redirect(zen_href_link('rewards'));
				die;
		}
		$model_new=zm_get_products_model($_POST['products_id']);
			
		if($_SESSION['cart']->count_contents()>0){
			$id_array=$_SESSION['cart']->get_product_id_array();
			$model_old=zm_get_products_model($id_array[0]);
			if(!in_array(REWARDS_PARENT_ID,$_SESSION['real_menu'])){
				$messageStack->add_session('header',NO_CARDS_WITH_FOOD,'error');
				zen_redirect(zen_href_link('rewards'));
				die;
			}
			if($model_new!=$model_old){
				$messageStack->add_session('header',NO_CARDS_WITH_CARDS,'error');
				zen_redirect(zen_href_link('rewards'));
				die;
			}
		}
		
		if($model_new=='GIFT_CARD' || $model_new=='E_GIFT_CARD'|| $model_new=='OTHER_GIFT_CARD'){
			$op_id_val=$_POST['id']['117'];
			//print_r($op_id_val);
			$pr_price_sql = $db->Execute('SELECT options_values_price
			FROM `products_attributes`
			WHERE `options_values_id` ="'.$op_id_val.'"
			LIMIT 0 , 30');
			
			$pr_price = $pr_price_sql->fields['options_values_price']*$_POST['cart_quantity'];
			//die;
			//print_r($pr_price);
			//die;
			//$pr_price=zen_get_products_base_price($_POST['products_id']);
			$customers_points = GetCustomersRewardPoints($_SESSION['customer_id']);
			$redeem_ratio=.01;
			if($pr_price>($customers_points*$redeem_ratio)){
				$messageStack->add_session('header',NOT_ENOUGH_POINTS,'error');
				zen_redirect(zen_href_link('rewards'));
				die;
			}
		}
		$_SESSION['is_gift_card']=true;
	}else if($_POST['real_menu']==GIFT_CARD_PARENT_ID){
		...
		if($_SESSION['cart']->count_contents()>0){
			$id_array=$_SESSION['cart']->get_product_id_array();
			$model_old=zm_get_products_model($id_array[0]);
			if(!in_array(GIFT_CARD_PARENT_ID,$_SESSION['real_menu'])){
				$messageStack->add_session('header',NO_CARDS_WITH_FOOD,'error');
				zen_redirect(zen_href_link('rewards'));
				die;
			}
		...
		$_SESSION['is_gift_card_non_rewards']=true;
	}else{
		if(isset($_POST['real_menu'])){
			$news_cat_id = zen_categories_lookup($_POST['real_menu'],'parent_id');
			$statu = $db->Execute('select categories_status from categories where categories_id = '.$news_cat_id);
			if($statu->fields['categories_status']==0){
				$messageStack->add_session('header',"Sorry, the current restaurant is closed.",'error');
				zen_redirect(zen_href_link('index', 'cPath='.$news_cat_id));
				die;
			}
		}
	$new_cat_id = zen_categories_lookup($_POST['real_menu'],'parent_id');
	$is_avail = zm_check_cart_for_available_for_address($new_cat_id);
	if($_SESSION['cart']->count_contents()>0){
		foreach($_SESSION['real_menu'] as $real){
			$new_real[]= zen_categories_lookup($real,'parent_id');
		}

		if(!in_array($new_cat_id,$new_real)){
			$messageStack->add_session('header',ONLY_ONE_RESTAURANT,'error');
			zen_redirect(zen_href_link('index', zen_get_path($new_cat_id)));
			die;
		}
	}
	
	if($is_avail==0){
		$messageStack->add_session('header',RESTAURANT_TO_ADDRESS_UNAVAIL,'error');
		zen_redirect(zen_href_link('index', zen_get_path($new_cat_id)));	
		die;		
	}
	
	zm_times_are_good_cart($_POST['products_id'],$_POST['real_menu'],$new_cat_id);
	
	}
	//zm edit
	$virt=zm_get_products_virtual($_POST['products_id']);
	
	if($virt==1){
		$_SESSION['is_virtual']=1;
	}else{
		$_SESSION['is_virtual']=0;
	}
...
// eof: adjust new quantity to be same as current in stock
      ...
      if ((zen_get_products_quantity_order_max($_POST['products_id']) == 1 and $this->in_cart_mixed($_POST['products_id']) == 1)) {
        ...
        if ($the_list != '') {
		  ...
		  $_SESSION['real_menu'][$_POST['products_id']]=$_POST['real_menu'];
...
// display message if all is good and not on shopping_cart page
      if (DISPLAY_CART == 'false' && $_GET['main_page'] != FILENAME_SHOPPING_CART && $messageStack->size('shopping_cart') == 0 ) {
		...
		if($_SESSION['is_gift_card']){
			zen_redirect(zen_href_link('rewards'));
		}else if($_SESSION['is_gift_card_non_rewards']){
			zen_redirect(zen_href_link('giftcards'));
		}else{
			zen_redirect(zen_href_link('index', zen_get_path($new_cat_id)));
		}
