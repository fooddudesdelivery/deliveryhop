...
/////////auto login code/////////////
if($_GET['token'] && !isset($_SESSION['customer_id'])){
  //echo "test";exit;
    // Check if email exists
    $getUserData = "SELECT userid from customer_token where token='".$_GET['token']."'";
    $getUserData = $db->Execute($getUserData);
    /*echo "<pre/>";
    print_r($getUserData->fields['userid']);
    exit;*/
     if ($getUserData->RecordCount()) {
        $check_customer_query = "SELECT customers_id, customers_firstname, customers_lastname, customers_password,
                                    customers_email_address, customers_default_address_id,
                                    customers_authorization, customers_referral
                           FROM " . TABLE_CUSTOMERS . "
                           WHERE customers_id = ".$getUserData->fields['userid']."
                           AND COWOA_account != 1";

      //$check_customer_query  =$db->bindVars($check_customer_query, ':customers_id');
...
if($_SERVER['REMOTE_ADDR']=='73.23.221.9'){
...
if(isset($_GET['cPath'])){
	...
	if(count($splode)===4){
		$comm= $db->Execute('select comments from categories_description where comments not in ("na",""," ") and categories_id = '.$splode[3]);
		if(strlen($comm->fields['comments'])>3){
			$messageStack->add_session('header',$comm->fields['comments'],'error');
		}
	}
    if(count($splode)===3){
		$comm= $db->Execute('select comments from categories_description where comments not in ("na",""," ") and categories_id = '.$splode[2]);
		if(strlen($comm->fields['comments'])>3){
			$messageStack->add_session('header',$comm->fields['comments'],'error');
		}
	}
