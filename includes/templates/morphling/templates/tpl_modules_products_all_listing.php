...
<?php
  ...
  if ($products_all_split->number_of_rows > 0) {
    ...
    while (!$products_all->EOF) {
      ...
      $discount = pt_get_discount_amount($products_all->fields['products_id']);
      ...
      if (PRODUCT_ALL_LIST_IMAGE != '0') {
        if ($products_all->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
          ...
          $display_products_image = '<div class="product-img"><a href="' . zen_href_link(zen_get_info_page($products_all->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_all->fields['master_categories_id']) . '&products_id=' . $products_all->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $products_all->fields['products_image'], $products_all->fields['products_name'], IMAGE_PRODUCT_ALL_LISTING_WIDTH, IMAGE_PRODUCT_ALL_LISTING_HEIGHT, 'class="img-responsive"') . '</a>' . $sticker . '<a href="' . zen_href_link(zen_get_info_page($products_all->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_all->fields['master_categories_id']) . '&products_id=' . $products_all->fields['products_id']) . '" class="product-qv" data-toggle="pt-quickview">' . TEXT_QUICKVIEW . '</a></div>';
        }
      } else {
        $display_products_image = '';
      }
      ...
      if (PRODUCT_ALL_LIST_NAME != '0') {
        $display_products_name = '<h3 class="product-name"><a href="' . zen_href_link(zen_get_info_page($products_all->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_all->fields['master_categories_id']) . '&products_id=' . $products_all->fields['products_id']) . '">' . $products_all->fields['products_name'] . '</a>' . '</h3>';
      } else {
        $display_products_name = '';
      }

      $display_category_name = '<h4 class="product-cat">' . zen_get_categories_name_from_product($products_all->fields['products_id']) . '</h4>';

      $products_rating = pt_get_products_rating($products_all->fields['products_id']); 
      ...
      if ((PRODUCT_ALL_LIST_PRICE != '0' and zen_get_products_allow_add_to_cart($products_all->fields['products_id']) == 'Y')  and zen_check_show_prices() == true) {
        ...
        $display_products_price = '<h3 class="product-price">' . $products_price . (zen_get_show_product_switch($products_all->fields['products_id'], 'ALWAYS_FREE_SHIPPING_IMAGE_SWITCH') ? (zen_get_product_is_always_free_shipping($products_all->fields['products_id']) ? TEXT_PRODUCT_FREE_SHIPPING_ICON : '') : '') . '</h3>';
      } else {
...
// more info in place of buy now
      if (PRODUCT_ALL_BUY_NOW != '0' and zen_get_products_allow_add_to_cart($products_all->fields['products_id']) == 'Y') {
        if (zen_has_product_attributes($products_all->fields['products_id'])) {
          $link = '<a href="' . zen_href_link(zen_get_info_page($products_all->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_all->fields['master_categories_id']) . '&products_id=' . $products_all->fields['products_id']) . '" class="pt-button pt-button-vs">' . BUTTON_SELECT_OPTIONS_ALT . '</a>';
        } else {
//          $link= '<a href="' . zen_href_link(FILENAME_ALL_PRODUCTS, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products_all->fields['products_id']) . '">' . zen_image_button(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT) . '</a>';
          if (PRODUCT_ALL_LISTING_MULTIPLE_ADD_TO_CART > 0 && $products_all->fields['products_qty_box_status'] != 0) {
//            $how_many++;
            $link = '<div class="product-add-qty">' . TEXT_PRODUCT_ALL_LISTING_MULTIPLE_ADD_TO_CART . "<input type=\"text\" name=\"products_id[" . $products_all->fields['products_id'] . "]\" value=\"0\" size=\"4\" />" . '</div>';
          } else {
            $link = '<a href="' . zen_href_link(FILENAME_PRODUCT_ALL, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products_all->fields['products_id']) . '" class="pt-button pt-button-vs">' . BUTTON_IN_CART_ALT . '</a>';
          }
        }
      ...
      if (PRODUCT_ALL_LIST_DESCRIPTION > '0') {
        ...
        $display_products_description = '<p class="product-desc">' . stripslashes(zen_trunc_string($disp_text, PRODUCT_ALL_LIST_DESCRIPTION)) . '</p>';
      } else {
