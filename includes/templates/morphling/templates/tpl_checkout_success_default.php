...
if( !empty($_SESSION['form_submit_save']['h_contactless']) && $_SESSION['form_submit_save']['h_contactless'] != "1"){
    $sql = "update orders set contactless = '".$_SESSION['form_submit_save']['h_contactless']."' where orders_id=".$_SESSION['new_order_id'];
    $result = $db->Execute($sql);
    
    $notes = "INSERT INTO notes(order_id, note_type, note, made_by, viewed, admin_id, customer_id)
    VALUES
    ('".$_SESSION['new_order_id']."','0', '".$_SESSION['form_submit_save']['h_driver_notes']."','".$_SESSION["customer_id"]."','1','0','".$_SESSION["customer_id"]."')";
...
<h3>Your order number is: <?php echo $_SESSION['new_order_id'] ?></h3>
<?php  unset($_SESSION['new_order_id']); ?>
<!--bof -gift certificate- send or spend box-->
<?php
// only show when there is a GV balance
  if ($customer_has_gv_balance && false ) {
?>
