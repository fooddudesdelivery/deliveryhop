...
// class methods
    function quote($method = '') {
      global $order;
		...
		$title = MODULE_SHIPPING_FLAT_TEXT_TITLE;
		if($_SESSION['is_gift_card'] && !$_SESSION['fast_ship'] || $_SESSION['is_gift_card_non_rewards'] && !$_SESSION['fast_ship']){
			$delivery_fee=0;
			$title ='Standard Mail';
		}else if($_SESSION['is_gift_card'] && $_SESSION['fast_ship'] || $_SESSION['is_gift_card_non_rewards'] && $_SESSION['fast_ship']){
			$delivery_fee=10;
			$title ='Expidited Mail';
		}else{
			$delivery_fee = $restaurant_info[$categories_id]['delivery_fee'];
			/*if(!$delivery_fee || $delivery_fee==0){
				$delivery_fee=4.99;
			}*/
      if((!$delivery_fee || $delivery_fee==0) AND $_SESSION['delivery_fee_flag_'.$categories_id] == "Global"){
        $delivery_fee=4.99;
      }
			$title ='Delivery';
		}
