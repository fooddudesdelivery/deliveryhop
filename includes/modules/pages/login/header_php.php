    ...
    if (!$check_customer->RecordCount()) {
      ...
      if (!$ProceedToLogin) {
		...
		if(isset($_SESSION['address_separated'])){
			$new_address_id = zm_add_address_to_book_for_login();
			if(is_numeric($new_address_id)){
			 $_SESSION['sendto'] = $_SESSION['cart_address_id'] = $_SESSION['customer_default_address_id'] =$new_address_id;
			}else{
