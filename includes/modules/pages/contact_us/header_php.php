  ...
  if ($zc_validate_email and !empty($enquiry) and !empty($name) && $error == FALSE) {
   ...
   if ($antiSpam != '') {
	...
	$customer_phone= $check_customer->fields['customers_telephone'];
      ...
      $customer_phone= NOT_LOGGED_IN_TEXT;
    }
    ...
    $html_msg['CONTACT_US_OFFICE_FROM'] = OFFICE_FROM . ' ' . $name . '<br />' . OFFICE_PHONE . ' ' . $phone 
	. '<br />' . OFFICE_EMAIL . '(' . $email_address . ')';
    $html_msg['EXTRA_INFO'] = $extra_info['HTML'];
    // Send message
    zen_mail($send_to_name,'service@staging.fooddudesdelivery.com', EMAIL_SUBJECT, $text_message, $name, $email_address, $html_msg,'contact_us');
	//mail('service@staging.fooddudesdelivery.com','Contact Us',str_replace('</strong>','',str_replace('<strong>','',$text_message)));$send_to_email
