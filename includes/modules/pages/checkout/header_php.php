...
//print_r($_REQUEST);
	if($_REQUEST['stanexp']){
		if($_REQUEST['stanexp']=='stan'){
			$_SESSION['fast_ship']=false;
		}else{
			$_SESSION['fast_ship']=true;
		}
...
if(isset($_SESSION['new_zone_id']) && isset($_SESSION['sendto'])){
	$zone_id = $_SESSION['new_zone_id'];
	$sendv = $_SESSION['sendto'];
	$db->Execute("update address_book set entry_zone_id=$zone_id where address_book_id=$sendv");
...
if(isset($_SESSION['sendto'])){
	$main_send = $_SESSION['sendto'];
	if($main_send>0){
	  $zone_ids = $db->Execute("select entry_zone_id from address_book where address_book_id=$main_send");
	  if($zone_ids->fields['entry_zone_id']==0){
		  if($_SESSION['customer_zone_id']!=0){
			  $up_zone = $_SESSION['customer_zone_id'];
			  $db->Execute("update address_book set entry_zone_id =$up_zone where address_book_id=$main_send");
			  $new_zip_update=true;
		  }else{
			  $new_zip_update=false;
		  }
	  }
	}
}
  ...
  if (FEC_SPLIT_CHECKOUT == 'true' and $credit_covers == false) {
   ...
   zm_check_min_and_checkout_header();
  ...
  if ( !$_SESSION['shipping'] || ( $_SESSION['shipping'] && ($_SESSION['shipping'] == false) && (zen_count_shipping_modules() > 1) ) ) {
    ...
    if (!($messageStack->size('checkout_payment') > 0) && !($messageStack->size('checkout_shipping') > 0) && !($messageStack->size('redemptions') > 0) && $_SESSION['shipping']) {
		if(isset($_SESSION['fooddudestaging_login'])){
			/* print_r( zen_get_all_get_params());
			die;
			 */
		}
      zen_redirect(zen_href_link(FILENAME_CHECKOUT, zen_get_all_get_params(), 'SSL'));
		    ...
		    $categories_id_zmc=zm_check_cart();
if(isset($categories_id_zmc) && intval($categories_id_zmc)>0){
			$delivery_fee = $_SESSION['restaurant_info'][$categories_id_zmc]['delivery_fee'];
			/*if(!$delivery_fee || $delivery_fee==0){
				$delivery_fee=4.99;
			}*/
      /*echo "Here: $categories_id_zmc <br/>";
      print_r($_SESSION);*/
      if((!$delivery_fee || $delivery_fee==0) AND $_SESSION['delivery_fee_flag_'.$categories_id_zmc] == "Global"){
        $delivery_fee=4.99;
      }
			$_SESSION['shipping']['cost']=$delivery_fee;
}
  ...
  case 'update':
	...
	$_SESSION['add_tip'] = $_POST['add_tip'];
	$_SESSION['tip_updated']=1;
          ...
          if (isset($quote['error'])) {
            ...
            if ( (isset($quote[0]['methods'][0]['title'])) && (isset($quote[0]['methods'][0]['cost'])) ) {
              $_SESSION['shipping'] = array('id' => $_SESSION['shipping'],
                ...
                //(zen_href_link(FILENAME_CHECKOUT, 'fecaction=null', 'SSL'));
            }
          }
        } else {
...
//echo "Shipping Flag: ".$_SESSION['delivery_fee_flag_'.$categories_id_zmc];

/*global $db, $messageStack;
$zc_check_customer = $db->Execute("SELECT customers_id, customers_authorization, REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(customers_telephone,' ',''),'+',''),'-',''),')',''),'(','') as customers_telephone, customers_email_address from " . TABLE_CUSTOMERS . " WHERE customers_id=" . (int)$_SESSION['customer_id']);
$customers_authorization = $zc_check_customer->fields['customers_authorization'];

//if($customers_authorization == "3"){
if(!empty($zc_check_customer->fields['customers_id'])){
    $search = array("(",")","+","-", " ");
    $customers_telephone = str_replace($search, "", $zc_check_customer->fields['customers_telephone']);
    //REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(orders.customers_telephone,' ',''),'+',''),'-',''),')',''),'(','') as customers_telephone
    $sql_ = "SELECT * FROM ( ";
    $sql_ .= " SELECT customers.customers_email_address as c_customers_email_address,
    orders.customers_email_address as o_customers_email_address,
    REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(customers.customers_telephone,' ',''),'+',''),'-',''),')',''),'(','') as c_customers_telephone,
    REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(orders.customers_telephone,' ',''),'+',''),'-',''),')',''),'(','') as o_customers_telephone,
    orders.orders_id, orders.cc_number, orders.cc_expires"; 
    $sql_ .= " from " . TABLE_CUSTOMERS . " LEFT JOIN orders ON (".TABLE_CUSTOMERS.".customers_id=orders.customers_id)";
    $sql_ .= " WHERE customers_authorization = '3' ) as t WHERE 0=0 ";

    if(!empty($zc_check_customer->fields['customers_telephone']) && !empty($zc_check_customer->fields['customers_email_address'])){
        $sql_ .= " OR (c_customers_telephone='" . $customers_telephone."' OR c_customers_email_address='".$zc_check_customer->fields['customers_email_address']."' OR o_customers_telephone='" . $customers_telephone."' OR o_customers_email_address='".$zc_check_customer->fields['customers_email_address']."') ";
    }

    if(isset($_POST["braintree_cc_number"]) && !empty($_POST["braintree_cc_number"])){
        $braintree_cc_number = $_POST["braintree_cc_number"];
        $braintree_cc_expires_month = $_POST["braintree_cc_expires_month"];
        $braintree_cc_expires_year = $_POST["braintree_cc_expires_year"];

        $cc_expires_1 = $cc_expires_2 = $cc_part_1 = $cc_part_2 = "";

        $cc_expires_1 = $braintree_cc_expires_month.$braintree_cc_expires_year;
        $cc_expires_2 = (int)$braintree_cc_expires_month.$braintree_cc_expires_year;

        $cc_part_1 = substr($braintree_cc_number, 0, 4);
        $cc_part_2 = substr($braintree_cc_number, 8, 4);

        $cc = $cc_part_1."XX".$cc_part_2;

        $sql_ .= " OR (cc_number='" . $cc."' AND (cc_expires='".$cc_expires_1."' OR cc_expires='".$cc_expires_2."')) ";
    }
    
    $zc_check_customer_all = $db->Execute($sql_);


    //$zc_check_customer_all = $db->Execute("SELECT customers_id, customers_authorization, customers_telephone, customers_email_address from " . TABLE_CUSTOMERS . " INNER JOIN orders ON (".TABLE_CUSTOMERS.".customers_id=orders.customers_id) WHERE customers_authorization = '3' AND (customers_telephone='" . $zc_check_customer->fields['customers_telephone']."' OR customers_email_address='".$zc_check_customer->fields['customers_email_address']."')");

    if ($zc_check_customer_all->RecordCount() > 0 || $customers_authorization) {
        ...
        $subject ="Blacklist: Orders On ".date("m/d/Y")." At ".date("h:i:s");
        $message = $subject."<br />";
        
        $message .= "Customers Telephone:" .$zc_check_customer->fields['customers_telephone']."<br />";
        $message .= "Customers Email Address:" .$zc_check_customer->fields['customers_email_address']."<br />";

        if(!empty($cc)){
            $message .= "cc_number:" .$cc_number."<br />";
            $message .= "cc_expires:" .$cc_expires."<br />";
        }
        ...
        unset($_SESSION['customer_id']);
        $messageStack->add_session('header', "Your Credit Card Declined", 'error');
    }
}*/
