...
if(isset($_SESSION['restaurant_info'])){
	if(isset($_SESSION['deliverhop_login'])){
...
while($num_categories>$count_row){
    ...
    if($c_level==4){
        ...
        foreach ($subcat as $cat) {
			if(zen_get_category_name($cat, $_SESSION['languages_id']) !='Restaurant Misc' || $_SESSION['restaurant_login']==1 || $_SESSION['deliverhop_login']==1){
				$names = zen_get_category_name($cat, $_SESSION['languages_id']);
				if($names !='Fooddudes Misc' && $names !='Food Dudes Misc' || $_SESSION['deliverhop_login']==1){
    ...
    if($c_level==3 && !in_array($current_category,$closed_restaurants)){
        $the_list .= '<div class="restaurant-tab-info-container">';
		...
		if(!isset($view_all)){
			...
			$the_list .= number_format($restaurant_info[$current_category]['delivery_fee'], 2);
        	$the_list .= '</div>';
		}
	}
// zm edit  
