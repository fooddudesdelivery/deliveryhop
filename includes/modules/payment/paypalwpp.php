...
//    $this->zcLog('update_status', 'Checking whether module should be enabled or not.');
  ...
  function selection() {
    ...
    return array('id' => $this->code,
                 'module' => MODULE_PAYMENT_PAYPALWPP_TEXT_BUTTON_ALTTEXT.'<span style="font-size:11px; font-family: Arial, Verdana;"> ' . MODULE_PAYMENT_PAYPALWPP_MARK_BUTTON_TXT . '</span>');
  }
...
//      $order->info['total'] = zen_round($order->info['total'], 2);
  ...
  function _doRefund($oID, $amount = 'Full', $note = '') {
	...
	$stat = $db->Execute("select orders_status from orders where orders_id=$oID");
    $new_order_status = $stat->fields['orders_status'];//(int)MODULE_PAYMENT_PAYPALWPP_REFUNDED_STATUS_ID;
    $orig_order_amount = 0;
    $doPayPal = $this->paypal_init();
  ...
  function _doAuth($oID, $amt, $currency = 'USD') {
    ...
    $authAmt = $amt;
	$stat = $db->Execute("select orders_status from orders where orders_id=$oID");
    $new_order_status = $stat->fields['orders_status'];
