...
class ot_gv {
  ...
  function ot_gv() {
    ...
    if (IS_ADMIN_FLAG !== true) {
      ...
      $this->checkbox = '<p>' . ($this->user_has_gv_account($_SESSION['customer_id']) > 0 ? MODULE_ORDER_TOTAL_GV_USER_BALANCE . $currencies->format($this->user_has_gv_account($_SESSION['customer_id'])) : '')  . '<br />' . $this->user_prompt . '<br />' . '<input type="text" size="6" onkeyup="submitFunction()" name="cot_gv" value="' . number_format($_POST['cot_gv'], 2) . '" onfocus="if (this.value == \'' . number_format($_SESSION['cot_gv'], 2) . '\') this.value = \'\';" />' . '</p>';
    }
    $this->output = array();
    if (IS_ADMIN_FLAG === true) {
