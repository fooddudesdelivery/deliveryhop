...
// $tmp_street= $_POST['street_address_shipping'];
// $tmp_city= $_POST['city_shipping'];
// $tmp_zone_id= $_POST['zone_id_shipping'];
// $tmp_state= $_POST['state_shipping'];
// $tmp_postcode= $_POST['postcode_shipping'];
// $tmp_lastname= $_POST['lastname_shipping'];
// $tmp_firstname= $_POST['firstname_shipping'];
//
//
// $_POST['street_address_shipping']= $_POST['street_address'];
// $_POST['city_shipping']= $_POST['city'];
// $_POST['zone_id_shipping']= $_POST['zone_id'];
// $_POST['state_shipping']= $_POST['state'];
// $_POST['postcode_shipping']= $_POST['postcode'];
// $_POST['lastname_shipping']= $_POST['lastname'];
// $_POST['firstname_shipping']= $_POST['firstname'];
//
//
// $_POST['street_address']=$tmp_street;
// $_POST['city']=$tmp_city;
// $_POST['zone_id']=$tmp_zone_id;
// $_POST['state']=$tmp_state;
// $_POST['postcode']=$tmp_postcode;
// $_POST['lastname']=$tmp_lastname;
// $_POST['firstname']=$tmp_firstname;

	}
    // begin free / virtual required fields
    $firstname = zen_db_prepare_input($_POST['firstname_shipping']);
    $lastname = zen_db_prepare_input($_POST['lastname_shipping']);
    ...
    if (!isset($_GET['type']) || $_GET['type'] != 'free_virtual') {
      ...
      $street_address = zen_db_prepare_input($_POST['street_address_shipping']);
      if (ACCOUNT_SUBURB == 'true') $suburb = zen_db_prepare_input($_POST['suburb']);
      $postcode = zen_db_prepare_input($_POST['postcode_shipping']);
      $city = zen_db_prepare_input($_POST['city_shipping']);
      if (ACCOUNT_STATE == 'true') {
        $state = zen_db_prepare_input($_POST['state_shipping']);
        if (isset($_POST['zone_id_shipping'])) {
          $zone_id = zen_db_prepare_input($_POST['zone_id_shipping']);
        } else {
          $zone_id = false;
        }
      }
      $country = zen_db_prepare_input($_POST['zone_country_id_shipping']);
  ...
  // begin shipping
      ...
      if (enable_shippingAddress()) {
        ...
        if (ACCOUNT_GENDER == 'true') $gender_shipping = zen_db_prepare_input($_POST['gender']);
        if (ACCOUNT_COMPANY == 'true') $company_billing = zen_db_prepare_input($_POST['company']);
        $firstname_billing = zen_db_prepare_input($_POST['firstname']);
        $lastname_billing = zen_db_prepare_input($_POST['lastname']);
        $street_address_billing = zen_db_prepare_input($_POST['street_address']);
        if (ACCOUNT_SUBURB == 'true') $suburb_billing = zen_db_prepare_input($_POST['suburb']);
        $postcode_billing = zen_db_prepare_input($_POST['postcode']);
        $city_billing = zen_db_prepare_input($_POST['city']);
        if (ACCOUNT_STATE == 'true') {
          $state_billing = zen_db_prepare_input($_POST['state']);
          if (isset($_POST['zone_id'])) {
            $zone_id_billing = zen_db_prepare_input($_POST['zone_id']);
          } else {
            $zone_id_billing = false;
          }
        }
        $country_shipping = zen_db_prepare_input($_POST['zone_country_id']);
    //echo ' I SEE: country=' . $country . '&nbsp;&nbsp;&nbsp;state=' . $state . '&nbsp;&nbsp;&nbsp;zone_id=' . $zone_id;
        ...
        if (strlen($street_address_billing) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
          $error = true;
          $messageStack->add('no_account', ENTRY_STREET_ADDRESS_ERROR);
        }
        if (strlen($firstname_billing) < ENTRY_FIRST_NAME_MIN_LENGTH) {
          $error = true;
          $messageStack->add('no_account', ENTRY_FIRST_NAME_ERROR);
        }
        if (strlen($lastname_billing) < ENTRY_LAST_NAME_MIN_LENGTH) {
          $error = true;
          $messageStack->add('no_account', ENTRY_LAST_NAME_ERROR);
        }        
        // BEGIN PO Box Ban 1/1
        if (defined('PO_BOX_ERROR')) {
          if ( preg_match('/PO BOX/si', $street_address_billing) ) {
            $error = true;
            $messageStack->add('no_account', PO_BOX_ERROR);
          } else if ( preg_match('/POBOX/si', $street_address_billing) ) {
            $error = true;
            $messageStack->add('no_account', PO_BOX_ERROR);
          } else if ( preg_match('/P\.O\./si', $street_address_billing) ) {
            $error = true;
            $messageStack->add('no_account', PO_BOX_ERROR);
          } else if ( preg_match('/P\.O/si', $street_address_billing) ) {
            $error = true;
            $messageStack->add('no_account', PO_BOX_ERROR);
          } else if ( preg_match('/PO\./si', $street_address_billing) ) {
            $error = true;
            $messageStack->add('no_account', PO_BOX_ERROR);
          }
        }
        ...
        if (strlen($city_billing) < ENTRY_CITY_MIN_LENGTH) {
          $error = true;
          $messageStack->add('no_account', ENTRY_CITY_ERROR);
        }
        ...
        if (ACCOUNT_STATE == 'true') {
          ...
          if ($entry_state_has_zones_shipping == true) {
            $zone_query = "SELECT distinct zone_id, zone_name, zone_code
                           ...
                           WHERE zone_country_id = :zoneCountryID
                           AND " . 
                         ((trim($state_billing) != '' && $zone_id_billing == 0) ? "(upper(zone_name) like ':zoneState%' OR upper(zone_code) like '%:zoneState%') OR " : "") .
                          "zone_id = :zoneID
                           ORDER BY zone_code ASC, zone_name";

            $zone_query = $db->bindVars($zone_query, ':zoneCountryID', $country_shipping, 'integer');
            $zone_query = $db->bindVars($zone_query, ':zoneState', strtoupper($state_billing), 'noquotestring');
            $zone_query = $db->bindVars($zone_query, ':zoneID', $zone_id_billing, 'integer');
            ...
            if ($zone_shipping->RecordCount() > 1) {
              while (!$zone_shipping->EOF && !$found_exact_iso_match_shipping) {
                if (strtoupper($zone->fields['zone_code']) == strtoupper($state_billing) ) {
            ...
            if ($found_exact_iso_match_shipping) {
              $zone_id_billing = $zone_shipping->fields['zone_id'];
            ...
            if (strlen($state_billing) < ENTRY_STATE_MIN_LENGTH) {
              $error = true;
              $error_state_input_shipping = true;
              $messageStack->add('no_account', ENTRY_STATE_ERROR);
            }
        ...
        if (strlen($postcode_billing) < ENTRY_POSTCODE_MIN_LENGTH) {
          $error = true;
          $messageStack->add('no_account', ENTRY_POST_CODE_ERROR);
        }
    ...
    if ($error == true) {
      ...
      if (enable_shippingAddress()) {
        ...
        $sql_data_array = array(array('fieldName'=>'customers_id', 'value'=>$_SESSION['customer_id'], 'type'=>'integer'),
                                array('fieldName'=>'entry_firstname', 'value'=>$firstname_billing, 'type'=>'string'),
                                array('fieldName'=>'entry_lastname','value'=>$lastname_billing, 'type'=>'string'),
                                array('fieldName'=>'entry_street_address','value'=>$street_address_billing, 'type'=>'string'),
                                array('fieldName'=>'entry_postcode', 'value'=>$postcode_billing, 'type'=>'string'),
                                array('fieldName'=>'entry_city', 'value'=>$city_billing, 'type'=>'string'),
        ...
        if (ACCOUNT_COMPANY == 'true') $sql_data_array[] = array('fieldName'=>'entry_company', 'value'=>$company_billing, 'type'=>'string');
        if (ACCOUNT_SUBURB == 'true') $sql_data_array[] = array('fieldName'=>'entry_suburb', 'value'=>$suburb_billing, 'type'=>'string');
        if (ACCOUNT_STATE == 'true') {
          if ($zone_id_billing > 0) {
            $sql_data_array[] = array('fieldName'=>'entry_zone_id', 'value'=>$zone_id_billing, 'type'=>'integer');
            ...
            $sql_data_array[] = array('fieldName'=>'entry_state', 'value'=>$state_billing, 'type'=>'string');
          }
        }
