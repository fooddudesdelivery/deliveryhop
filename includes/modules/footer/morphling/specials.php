...
$random_specials_sidebox_product_query .= $limit;

//    $random_specials_sidebox_product = zen_random_select($random_specials_sidebox_product_query);
$random_specials_sidebox_product = $db->ExecuteRandomMulti($random_specials_sidebox_product_query, 3);

if ($random_specials_sidebox_product->RecordCount() > 0)  {
  $title =  BOX_HEADING_SPECIALS;
  $box_id = 'sale_widget';
  require($template->get_template_dir('tpl_specials.php',DIR_WS_TEMPLATE, $current_page_base,'footer'). '/tpl_specials.php'); 
}
?>
