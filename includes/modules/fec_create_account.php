...
if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  $country =223;//= zen_db_prepare_input($_POST['zone_country_id']);
...
// begin shipping
    if (enable_shippingAddress()) {
      ...
      if (ACCOUNT_COMPANY == 'true') $company_shipping = zen_db_prepare_input($_POST['company']);
      $firstname_shipping = zen_db_prepare_input($_POST['firstname']);
      $lastname_shipping = zen_db_prepare_input($_POST['lastname']);
      $street_address_shipping = zen_db_prepare_input($_POST['street_address']);
      if (ACCOUNT_SUBURB == 'true') $suburb_shipping = zen_db_prepare_input($_POST['suburb']);
      $postcode_shipping = zen_db_prepare_input($_POST['postcode']);
      $city_shipping = zen_db_prepare_input($_POST['city']);
      if (ACCOUNT_STATE == 'true') {
        $state_shipping = zen_db_prepare_input($_POST['state']);
        if (isset($_POST['zone_id'])) {
          $zone_id_shipping = zen_db_prepare_input($_POST['zone_id']);
        } else {
...
// end shipping  
  ...
  if ($error == true) {
    ...
    if (enable_shippingAddress()) {
      ...
      $_SESSION['billto'] = $_SESSION['cart_address_id'] = $db->Insert_ID();
    } else {
      // FEC MODIFICATION
      $_SESSION['billto'] = $_SESSION['cart_address_id'] = (int)$address_id;
    }
    $_SESSION['shipping'] = '';
...
//echo "update address_book set entry_zone_id=$zone_id where address_book_id=$sendv";
//die;
        unset($_SESSION['shipping']);
		$_SESSION['address_separated'][0]['street_number']=$company.' '.$street_address;
		$_SESSION['address_separated'][0]['street']=$suburb;
		$_SESSION['address_separated'][0]['city']=$city;
		$_SESSION['address_separated'][0]['state']=$new_state;
		$_SESSION['address_separated'][0]['postcode']=$postcode;
		$_SESSION['address_separated'][0]['apt']=$suburb;
		$_SESSION['customer_first_name']=$firstname;
		$_SESSION['customer_last_name']=$lastname;
		$_SESSION['firstname_shipping']=$firstname;
		$_SESSION['lastname_shipping']=$lastname;
		$_SESSION['firstname']=$firstname;
		$_SESSION['lastname']=$lastname;
		$_SESSION['company_shipping']=$company;
		$_SESSION['street_address_shipping']=$street_address;
		$_SESSION['suburb_shipping']=$suburb;
		$_SESSION['city_shipping']=$city;
