...
// TABLES: customers, address_book
  ...
  function zm_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n") {
    global $db;
    $address_query = "select 
                              entry_street_address as street_address, entry_city as city, 
                             entry_state as state,entry_suburb as apt
                      from " . TABLE_ADDRESS_BOOK . "
                      where customers_id = '" . (int)$customers_id . "'
                      and address_book_id = '" . (int)$address_id . "'";
    ...
    return $address->fields['street_address'].' '.$address->fields['apt'].' '.$address->fields['city'].' '.$address->fields['state'];
  }
  
function zm_address_label_checkout($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n") {
    global $db;
    $address_query = "select entry_firstname as firstname, entry_lastname as lastname,
                             entry_company as company, entry_street_address as street_address,
                             entry_suburb as suburb, entry_city as city, entry_postcode as postcode,
    ...
    return $address->fields['company'].'<br />'.$address->fields['firstname'].' '.$address->fields['lastname'].
	'<br />'.$address->fields['street_address'].' '.$address->fields['suburb'].
	'<br />'.' '.$address->fields['city'].', '.$address->fields['state'].' '.$address->fields['postcode'];
}
